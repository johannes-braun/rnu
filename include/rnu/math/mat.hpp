...
namespace rnu
{
  ...
  public:
    ...
    constexpr mat(Ts&&... values) requires ((detail::count_v<Ts> == 1) && ...) && (sizeof...(Ts) == R * C) && (std::is_convertible_v<Ts, T> && ...)
      : _columns(columnize({ static_cast<T>(std::forward<Ts>(values))... })) {
    ...
    : mat(detail::traverse<C, R>([&](size_t c, size_t r) {
      ...
      [[nodiscard]] constexpr pointer data() { return this->_columns[0].data(); }
      [[nodiscard]] constexpr const_pointer data() const { return this->_columns[0].data(); }
  ...
  private:
    ...
    constexpr std::array<column_type, C> columnize(std::array<T, R* C> const& src, std::index_sequence<RowIndex...>)
    {
      std::array<column_type, C> result{};
      for (size_t i = 0; i < C; ++i)
      {
        result[i] = column_type{ src[i * R + RowIndex]... };
      }
      return result;
    }

    constexpr std::array<column_type, C> columnize(std::array<T, R* C> const& src)
    {
      return columnize(src, std::make_index_sequence<R>());
    }

    template<typename V, size_t... Indices>
    constexpr mat(fill_sentinel_t, V&& v, std::index_sequence<Indices...>)
      : _columns(columnize({ (Indices, v)... }))
    {
    ...
    constexpr mat(diag_sentinel_t, std::index_sequence<Indices...>, V&& v)
      : _columns(columnize({ detail::value_if_diag<Indices, V, C, R, T>(std::forward<V>(v))... }))
    {
    ...
    constexpr mat(diag_fill_sentinel_t, std::index_sequence<Indices...>, std::tuple<V...> v)
      : _columns(columnize({ detail::value_if_diag_va<Indices, C, R, T>(v)... }))
    {
    ...
    constexpr mat(fill_all_sentinel_t, std::index_sequence<Indices...>, V* v)
      :_columns(columnize({ static_cast<T>(v[Indices])... }))
    {
