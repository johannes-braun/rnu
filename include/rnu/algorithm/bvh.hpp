...
struct aabb_t {
  ...
  [[nodiscard]] constexpr aabb_t intersect(aabb_t const& other) const noexcept {
    return aabb_t{rnu::max(min, other.min), rnu::min(max, other.max)};
  }

  [[nodiscard]] constexpr std::pair<std::optional<aabb_t>, std::optional<aabb_t>> split(int axis, float position) {
  ...
  constexpr bool empty(float eps = 1e-7) const noexcept {
...
struct ray_t {
  ...
  [[nodiscard]] std::optional<float> intersect(
...
[[nodiscard]] std::vector<aabb_t> generate_triangle_bounds(
...
private:
  ...
  [[nodiscard]] std::optional<std::pair<bvh_node_t, bvh_node_t>> split(
