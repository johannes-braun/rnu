...
namespace rnu
{
  ...
  struct perspective_projection
  {
    ...
    [[nodiscard]] constexpr perspective_projection(float_type fovy_radians = rnu::radians(60.0f),
      float_type aspect = 1.0f, float_type near = 0.01f, float_type far = 100.f)
    {
      this->fovy_radians = fovy_radians;
      this->aspect = aspect;
      this->near = near;
      this->far = far;
    }
  ...
  struct ortho_projection
  {
    ...
    [[nodiscard]] constexpr ortho_projection(float_type left = -1.0f, float_type right = 1.0f, float_type top = -1.0f,
      float_type bottom = 1.0f, float_type near = 0.01f, float_type far = 100.f)
    {
      this->left = left;
      this->right = right;
      this->top = top;
  ...
  template<typename Float>
  using projection = std::variant<perspective_projection<Float>, ortho_projection<Float>>;

  template<typename Float>
  constexpr Float near_plane(projection<Float> const& p)
  {
    struct
    {
      constexpr Float operator()(perspective_projection<Float> const& persp) const
      {
        return persp.near;
      }
      constexpr Float operator()(ortho_projection<Float> const& ortho) const
      {
        return ortho.near;
      }
    } get;
    return std::visit(get, p);
  }

  template<typename Float>
  constexpr Float far_plane(projection<Float> const& p)
  {
    struct
    {
      constexpr Float operator()(perspective_projection<Float> const& persp) const
  ...
  template<typename Float>
  constexpr auto projection_matrix(projection<Float> const& p)
  {
    struct
    {
      constexpr auto operator()(perspective_projection<Float> const& persp) const
      {
        return persp.matrix();
      }
      constexpr auto operator()(ortho_projection<Float> const& ortho) const
      {
        return ortho.matrix();
      }
    } get;
  ...
  public:
    ...
    [[nodiscard]] constexpr static mat_type projection(float_type fovy_radians, float_type aspect, float_type near,
      float_type far, bool row_major = false, bool y_invert = false) noexcept
    {
      perspective_projection<Float> const result{fovy_radians, aspect, near, far};
    ...
    {
      return ortho_projection<Float>{left, right, top, bottom, near, far}.matrix();
    }
